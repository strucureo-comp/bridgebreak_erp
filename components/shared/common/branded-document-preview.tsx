...
export function BrandedDocumentPreview({ 
    ...
    const formatCurrency = (amount: number) => {
        return new Intl.NumberFormat('en-AE', {
            style: 'currency',
            currency: currency || companyProfile?.baseCurrency || 'AED',
        }).format(amount);
    };
    ...
    return (
        <div className={cn(
            ...
            <div className={cn(
                ...
                <div className={cn(
                    ...
                    <div className="space-y-1">
                        ...
                        <p className="text-xs font-bold">{format(new Date(date), 'dd MMMM yyyy')}</p>
                        {validUntil && (
                            <>
            ...
            <div className="grid grid-cols-2 gap-12 mb-12">
                <div className="space-y-4">
                    <h3 className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] border-b pb-1">
                        {type === 'po' ? 'Supplier Entity' : 'Client Entity'}
                    </h3>
                    <div className="space-y-1">
                        <p className="text-sm font-black uppercase">{entityName || 'TARGET-ENTITY-NOT-SELECTED'}</p>
                        <p className="text-[10px] text-muted-foreground">{entityAddress || 'Entity address not provided'}</p>
                        {entityTaxId && (
                ...
                <div className="space-y-4 text-right">
                    <h3 className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] border-b pb-1">
                        {type === 'po' ? 'Ship To / Site' : 'Payment Terms'}
                    </h3>
                    <div className="space-y-1">
                        {type === 'po' ? (
                            <>
                            ...
                            <>
                                <p className="text-sm font-black uppercase">NET 30 DAYS</p>
                                <p className="text-[10px] text-muted-foreground">Subject to Credit Approval</p>
                                <p className="text-[10px] font-bold text-foreground mt-2">BANK TRANSFER ONLY</p>
                            </>
                        )}
                    </div>
                </div>
            ...
            <div className="mt-8 flex justify-end">
                <div className="w-64 space-y-3">
                    ...
                    <div className="flex justify-between items-center bg-foreground text-card-foreground p-4 rounded-sm mt-4">
                        <span className="text-[10px] font-black uppercase tracking-widest">Total Position</span>
            ...
            <div className="mt-12 grid grid-cols-2 gap-12 pt-8 border-t border-border">
                <div className="space-y-2">
                    <h4 className="text-[9px] font-black uppercase tracking-widest text-muted-foreground">Notes & Policy</h4>
                    <p className="text-[8px] text-muted-foreground leading-relaxed uppercase font-medium whitespace-pre-line">
                        {branding?.terms || 'Standard terms and conditions apply to all transactions.'}
                    </p>
                </div>
                <div className="flex flex-col items-end justify-end space-y-2">
                    ...
                    <p className="text-[9px] font-black uppercase">{companyProfile?.legalName || 'Authorized Signatory'}</p>
                </div>
            </div>
