...
export default function PaymentVouchersPage() {
  ...
  const [form, setForm] = useState<PaymentVoucher>({ ...EMPTY, currency: currencyCode });
  ...
  const normalized = (doc: any): PaymentVoucher => ({
    ...doc,
    id: doc.id || doc._id,
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        toast.error(error?.message || 'Failed to load payment vouchers');
      } finally {
        setLoading(false);
      }
    };
  ...
  const filtered = useMemo(
    () =>
      items.filter((voucher) =>
        [voucher.voucherNumber, voucher.payeeName, voucher.referenceNo]
          .filter(Boolean)
  ...
  const save = async () => {
    ...
    try {
      if (form.id) {
        const updated = normalized(await updatePaymentVoucher(form.id, payload));
        setItems((prev) => prev.map((item) => (item.id === form.id ? updated : item)));
      } else {
        const created = normalized(await createPaymentVoucher(payload));
  ...
  const approve = async (id: string) => {
    try {
      setActionId(id);
      const updated = updateFromAction(await approvePaymentVoucher(id));
  ...
  const post = async (id: string) => {
    try {
      setActionId(id);
      const updated = updateFromAction(await postPaymentVoucher(id));
      setItems((prev) => prev.map((item) => (item.id === id ? updated : item)));
      toast.success('Payment voucher posted to ledger');
    } catch (error: any) {
      toast.error(error?.message || 'Failed to post payment voucher');
    } finally {
      setActionId('');
    }
  };
      ...
      <div className="mx-auto max-w-6xl space-y-6 pb-10">
        <div className="flex items-center justify-between border-b pb-5">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Approve and post outgoing payments to the ledger.</p>
          </div>
                ...
                <CardContent className="flex flex-col gap-4 p-4 md:flex-row md:items-center md:justify-between">
                  <div className="space-y-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                      {voucher.payeeName} • {voucher.date}
                    </p>
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogContent className="max-w-3xl">
            <DialogHeader>
              <DialogTitle>{form.id ? 'Edit' : 'Create'} Payment Voucher</DialogTitle>
            </DialogHeader>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setForm((prev) => ({ ...prev, paymentMethod: value as PaymentVoucher['paymentMethod'] }))}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
              ...
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                <div>
                  ...
                  <Select value={form.payeeType} onValueChange={(value) => setForm((prev) => ({ ...prev, payeeType: value as PaymentVoucher['payeeType'] }))}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div>
                  ...
                  <Input value={form.payeeName} onChange={(e) => setForm((prev) => ({ ...prev, payeeName: e.target.value }))} />
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                {form.lines.map((line, index) => (
                  <div key={index} className="space-y-1 rounded-lg border p-3">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {line.accountCode ? accountsByCode.get(line.accountCode)?.name || 'Account not found' : 'Select the debit account for this payment line.'}
                    </p>
                  </div>
                ))}
              </div>
