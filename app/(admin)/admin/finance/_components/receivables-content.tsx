...
import { getReceivables, createReceivable, getCustomers } from '@/lib/api';
import type { Invoice, CustomerAccount } from '@/lib/db/types';
import type { TaxRateDefinition } from '@/app/(admin)/admin/finance/_lib/finance/tax-engine';
...
export function ReceivablesContent() {
    ...
    useEffect(() => {
        if (!formData.invoice_number) {
            ...
            setFormData(prev => ({ ...prev, invoice_number: `INV-${year}-${random}` }));
        }
    ...
    const stats = useMemo(() => {
        ...
        const totalAR = filtered
            .filter(i => i.posting_status === 'posted' && i.status !== 'paid')
            .reduce((sum, i) => sum + Number(i.total_amount || i.amount), 0);
        const overdue = filtered
            .filter(i => i.posting_status === 'posted' && i.status !== 'paid' && new Date(i.due_date) < new Date())
            .reduce((sum, i) => sum + Number(i.total_amount || i.amount), 0);
        const postedCount = filtered.filter(i => i.posting_status === 'posted').length;
        const draftCount = filtered.filter(i => i.posting_status === 'draft').length;

        return { totalAR, overdue, postedCount, draftCount };
    }, [invoices, activeTab]);

    // Calculate tax for a line item
    const calculateLineTax = async (item: InvoiceLineItem, countryCode: string, isExempt: boolean) => {
        if (isExempt || !item.taxRateId) {
            return { taxAmount: 0, total: item.quantity * item.unitPrice };
        }
    ...
    // Recalculate all taxes when exemption status changes
    useEffect(() => {
    ...
    const handleCreate = async () => {
        try {
            ...
            await createReceivable(invoiceData);
                    ...
                    <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
                        ...
                        <DialogContent className="sm:max-w-[900px] max-h-[90vh] overflow-y-auto rounded-[2rem]">
                            <DialogHeader>
                                ...
                                <DialogDescription>
                                    Create a detailed invoice with line items and automatic tax calculation for any country.
                                </DialogDescription>
                            </DialogHeader>
                                        ...
                                        <SelectContent className="rounded-xl">
                                            {customers.map(customer => (
                                                <SelectItem key={customer.id} value={customer.id}>
                                                    {customer.name} {customer.tax_id && `(Tax ID: ${customer.tax_id})`}
                                                </SelectItem>
                                            ))}
                                        </SelectContent>
                                    </Select>
                                        ...
                                        return (
                                            <TableRow key={inv.id} className="hover:bg-slate-50/50">
                                                ...
                                                <TableCell className="text-right font-bold">
                                                    ${Number(inv.total_amount || inv.amount).toLocaleString()}
                                                </TableCell>
