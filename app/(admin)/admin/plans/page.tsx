        ...
        return (
            <DashboardShell requireAdmin>
                <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-4">
                    ...
                    <p className="font-bold text-foreground">Opening Notebook...</p>
                </div>
            </DashboardShell>
        );
    }
            ...
            <div className="space-y-8 pb-12 h-[calc(100vh-8rem)] flex flex-col">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 shrink-0">
                    <div className="space-y-1">
                        <h1 className="text-4xl font-black tracking-tight text-foreground">Idea Notebook</h1>
                        <p className="text-muted-foreground font-medium">Draft strategies, mission goals, and team ideas.</p>
                    </div>
                    {!isEditing && (
                ...
                <div className="flex-1 flex flex-col md:flex-row gap-8 min-h-0">
                    ...
                    <div className={cn(
                        ...
                        <div className="flex items-center justify-between px-2">
                            <h2 className="text-sm font-black text-muted-foreground uppercase tracking-widest">All Entries</h2>
                            <Badge className="bg-muted text-muted-foreground border-none font-black">{notes.length}</Badge>
                        </div>
                            ...
                            {notes.map(note => (
                                <Card 
                                    ...
                                    className={cn(
                                        ...
                                        selectedNoteId === note.id ? "bg-slate-900 text-card-foreground shadow-xl shadow-slate-200" : "bg-card hover:bg-slate-50"
                                    )}
                                >
                                    <CardContent className="p-6 space-y-3">
                                        <div className="flex items-start justify-between">
                                            ...
                                            <Button variant="ghost" size="icon" className="h-6 w-6 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity hover:bg-rose-500/20 text-muted-foreground hover:text-rose-500" onClick={(e) => handleDelete(note.id, e)}>
                                                <Trash2 size={12} />
                                            </Button>
                                        </div>
                                        ...
                                        <p className={cn(
                                            ...
                                            selectedNoteId === note.id ? "text-muted-foreground" : "text-muted-foreground"
                                        )}>
                                            {note.content}
                                        </p>
                                    </CardContent>
                    ...
                    <div className={cn(
                        ...
                        {isEditing ? (
                            <Card className="flex-1 rounded-[3rem] border-none shadow-sm bg-card overflow-hidden flex flex-col border-4 border-slate-50">
                                <div className="p-8 border-b border-slate-50 flex items-center justify-between bg-card shrink-0">
                                    <div className="flex items-center gap-4 flex-1">
                                        <Button variant="ghost" size="icon" className="md:hidden rounded-xl bg-muted" onClick={() => setIsEditing(false)}>
                                            <ChevronLeft size={20} />
                                        </Button>
                                    ...
                                    <div className="flex items-center gap-3">
                                        <Select value={editorCategory} onValueChange={(v: any) => setEditorCategory(v)}>
                                            <SelectTrigger className="w-[120px] h-10 rounded-xl font-black text-[10px] uppercase tracking-widest border-border">
                                                <SelectValue />
                                            </SelectTrigger>
                                ...
                                <div className="flex-1 overflow-hidden relative">
                                    <Textarea
                                        ...
                                        className="w-full h-full resize-none border-none focus-visible:ring-0 p-10 text-lg leading-relaxed font-medium text-muted-foreground placeholder:text-slate-100"
                                    />
                                </div>
                            </Card>
                        ) : (
                            <div className="flex-1 flex flex-col items-center justify-center bg-slate-50/50 rounded-[3rem] border-4 border-dashed border-border p-12 text-center space-y-6">
                                <div className="h-24 w-24 rounded-[2.5rem] bg-card flex items-center justify-center text-slate-100 shadow-sm">
                                    <Book size={48} strokeWidth={1.5} />
                                </div>
                                <div className="max-w-xs space-y-2">
                                    <h3 className="text-2xl font-black text-foreground">Empty Page</h3>
                                    <p className="text-muted-foreground font-medium leading-relaxed">Select an entry from the sidebar to continue drafting or start a new one.</p>
                                </div>
                                <Button onClick={handleCreateNew} variant="outline" className="rounded-2xl border-border h-12 px-10 font-bold hover:bg-white hover:shadow-md transition-all">
